...
**What exactly is taken out from under the penalty.** All the usual partitions occur in the corpus: weight matrices only — <i>"decoupled weight decay $\lambda$ applied to weight matrices only"</i> \[[1.2](#ref-1-2)\]; everything except biases and norms \[[3.2](#ref-3-2)\]; token and position embeddings excluded "following standard practice" \[[3.1](#ref-3-1)\]. The difference is not cosmetic: in one of the works the norm is defined over exactly the same set the penalty acts on — <i>"weight decay and the clamp act on $E,W_{1},W_{2}$; biases are unregularized and excluded from $\Vert W\Vert$"</i> \[[1.3](#ref-1-3)\] — so the quantity by which the delay is measured depends on the partition just as the delay itself does.
