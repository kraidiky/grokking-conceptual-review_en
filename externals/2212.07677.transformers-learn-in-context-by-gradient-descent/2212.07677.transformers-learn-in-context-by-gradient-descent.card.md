...
**Proposition 1 differs in its indices between the main text and the appendix.** In the [main formulation](#p4-2) the dynamics is written as $(x_{j},y_{j})+P\mkern3mu VK^{T}q_{j}$, whereas in the repetition of the proposition in [A.1](#sec-a1) the first pair carries the index $i$: $(x_{i},y_{i})+P\mkern3mu VK^{T}q_{j}$. A typo, but it stands in the formulation of the central proposition; when citing the formula one should check it against the derivation in A.1.
