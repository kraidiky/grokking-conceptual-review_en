...
**The RLCT surrogate is honestly called a surrogate.** $\lambda_{\text{proxy}}=\Vert\mathbf{W}\Vert_{0}/2$ is an upper bound only in the regular case, and in a genuinely singular setting the true RLCT may be much smaller ([`p7-4`](#p7-4)). Note that a neighbouring work of the corpus (Cullen et al. 2026) computes the RLCT in closed form rather than replacing it with a support count.
