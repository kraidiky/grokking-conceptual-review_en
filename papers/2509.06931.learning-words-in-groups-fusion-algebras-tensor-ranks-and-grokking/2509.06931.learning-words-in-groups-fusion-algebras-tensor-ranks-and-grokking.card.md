...
**[Fourier features](../../concepts/fourier-features-circuits.md)** — Gromov's Fourier ansatz is explained as the case $G=\mathbb{Z}_ {p}$ of a general bsc decomposition; in the case of multiplication the support is diagonal, $\lbrace(\phi,\phi,\phi)\rbrace$, and the network finds Strassen-like low-rank realisations of the bsc projections: the empirical minimum width coincides exactly with the theoretical rank bound ($D_{16}$ — 7 against 7, $\mathbb{Z}_ {32}$ — 3 against $\le 3$, $Q_{8}$ — 8 against 8) ([`p18-1`](#p18-1), [`fig-10`](#fig-10)) — a rare case in which a prediction of the mechanism is checked by a number.
