...
**How the work will be cited** ("a trade of data against time at grokking is proved") — more precisely: what is proved is an SQ barrier tying $r,T,\tau,\delta$, and a data–width trade; the data–time trade is an experimental observation on a grid $n\le300$, $k\in\{3,4\}$ at fixed $\eta,\lambda,B$ and a cut-off $T=10^{5}$.
